...
| `runtimeVersion` _string_ | RuntimeVersion is the Dynamo runtime version for this component. The operator<br />uses it to determine compatibility skew support with the operator version.<br />When omitted on create, or on update when the runtimeVersion path is unchanged<br />and the image path changes, the operator defaults runtimeVersion from a<br />parseable image tag. For standalone DynamoComponentDeployment objects, these<br />paths are `.spec.runtimeVersion` and `.spec.extraPodSpec.mainContainer.image`.<br />For DynamoGraphDeployment services, these paths are<br />`.spec.services.<serviceName>.runtimeVersion` and<br />`.spec.services.<serviceName>.extraPodSpec.mainContainer.image`. For example,<br />`vllm-runtime:1.1.0` defaults runtimeVersion to `1.1.0`.<br />Set this explicitly when using SHA-tagged or custom runtime images. |  | Pattern: `^(0\|[1-9][0-9]*)\.(0\|[1-9][0-9]*)\.(0\|[1-9][0-9]*)$` <br />Optional: \{\} <br /> |
...
| `runtimeVersion` _string_ | runtimeVersion is the Dynamo runtime version for this component. The operator<br />uses it to determine compatibility skew support with the operator version.<br />When omitted on create, or on update when the runtimeVersion path is unchanged<br />and the image path changes, the operator defaults runtimeVersion from a<br />parseable image tag. For standalone DynamoComponentDeployment objects, these<br />paths are `.spec.runtimeVersion` and<br />`.spec.podTemplate.spec.containers[?(@.name=="main")].image`. For<br />DynamoGraphDeployment components, these paths are<br />`.spec.components[*].runtimeVersion` and<br />`.spec.components[*].podTemplate.spec.containers[?(@.name=="main")].image`.<br />For example, `vllm-runtime:1.1.0` defaults runtimeVersion to `1.1.0`.<br />Set this explicitly when using SHA-tagged or custom runtime images. |  | Pattern: `^(0\|[1-9][0-9]*)\.(0\|[1-9][0-9]*)\.(0\|[1-9][0-9]*)$` <br />Optional: \{\} <br /> |
