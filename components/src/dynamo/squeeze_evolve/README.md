...
Run it from a source checkout. The tier workers run on `dynamo.vllm`, so install the vLLM extra alongside this component:

```bash
git clone https://github.com/ai-dynamo/dynamo
cd dynamo
(cd lib/bindings/python && maturin develop --uv)   # build the Rust bindings
uv pip install -e '.[squeeze_evolve,vllm]'
```
