...
/// * otherwise the pool starts at 10000 and may be updated once from reported
///   engine capacity. The TCP queue defaults to 16.
#[derive(Debug)]
struct SizingConfig {
    pool_size: usize,
    queue_size: usize,
...
fn resolve_sizing(env: impl Fn(&str) -> Result<Option<usize>>) -> Result<SizingConfig> {
    ...
    let sizing = match read_capacity("DYN_ENGINE_REQUEST_LIMIT", 1)? {
        ...
        None => {
            ...
            SizingConfig {
                pool_size: explicit_pool_size.unwrap_or(DEFAULT_WORKER_POOL_SIZE),
...
pub struct SharedTcpServer {
    ...
    /// Pool size selected before any engine capacity hint was available.
...
impl SharedTcpServer {
    pub fn new(bind_addr: SocketAddr, cancellation_token: CancellationToken) -> Result<Arc<Self>> {
        let sizing = resolve_sizing(parse_env_usize)?;
...
mod tests {
    ...
    fn assert_sizing_error<const N: usize>(
        ...
        let error = resolve_sizing(sizing_env(values)).unwrap_err().to_string();
        assert!(
    ...
    fn worker_pool_size_precedence() {
        ...
        let sizing = resolve_sizing(sizing_env([
        ...
        assert!(!sizing.accepts_engine_capacity_hint);
        let sizing = resolve_sizing(sizing_env([("DYN_ENGINE_REQUEST_LIMIT", 7)])).unwrap();
        assert_eq!(sizing.pool_size, 7);
        ...
        let fallback = resolve_sizing(|_| Ok(None)).unwrap();
    ...
    async fn late_engine_capacity_updates_only_the_fallback_pool() {
        ...
        let server = SharedTcpServer::new_with_sizing(
            ...
            SizingConfig {
                ...
                accepts_engine_capacity_hint: true,
            },
        );
        ...
        let overridden = SharedTcpServer::new_with_sizing(
            bind_addr,
            cancellation_token.clone(),
            SizingConfig {
                pool_size: 13,
    ...
    async fn test_capacities_published_on_server_init() {
        ...
        let cancellation_token = CancellationToken::new();
        let bind_addr: SocketAddr = "127.0.0.1:0".parse().unwrap();
        let sizing = resolve_sizing(|_| Ok(None)).unwrap();
        let _server =
