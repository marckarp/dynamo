...
type DynamoComponentDeploymentSharedSpec struct {
	...
	// paths are `.spec.runtimeVersion` and `.spec.extraPodSpec.mainContainer.image`.
	// For DynamoGraphDeployment services, these paths are
	// `.spec.services.<serviceName>.runtimeVersion` and
	// `.spec.services.<serviceName>.extraPodSpec.mainContainer.image`. For example,
	// `vllm-runtime:1.1.0` defaults runtimeVersion to `1.1.0`.
